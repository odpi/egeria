...
public class TestSupportedRelationshipLifecycle extends RepositoryConformanceTestCase
{
    ...
    private static final String assertionMsg32 = " repository created entity proxies.";

    private static final String assertion33   = testCaseId + "-33";
    private static final String assertionMsg33 = " repository retrieved entity proxies.";

    private static final String assertion34   = testCaseId + "-34";
    private static final String assertionMsg34 = " repository created relationship between entity proxies.";
    ...
    protected void run() throws Exception
    {
        ...
        /*
         * Repeat a subset of the above tests where we only have entity proxy instances - no full local entity instances.
         */
        ...
        String proxy1GUID = null;
        String proxy2GUID = null;
        ...
        try {
            ...
            //EntityDef end1Type = entityDefs.get(end1TypeName);
            String homeMetadataColelctionId = "another-repository";  // This does need to be a real repository
            proxy1GUID = this.addEntityProxyToRepository(workPad.getLocalServerUserId(), metadataCollection, end1Type, homeMetadataColelctionId);
            //EntityDef end2Type = entityDefs.get(end2TypeName);
            proxy2GUID = this.addEntityProxyToRepository(workPad.getLocalServerUserId(), metadataCollection, end2Type, homeMetadataColelctionId);
        ...
        /*
         * If we created proxies - check we can retrieve them and then try to create a relationship that uses them.
         */

        if (proxy1GUID != null && proxy2GUID != null) {

            /*
             * While we're here - check we can retrieve a proxy (as an EntitySummary)
             */
            EntitySummary retrievedEntSummary1 = metadataCollection.getEntitySummary(workPad.getLocalServerUserId(), proxy1GUID);

            assertCondition((retrievedEntSummary1 != null),
                    assertion33,
            ...
            try {
                ...
                pRelationship = metadataCollection.addRelationship(workPad.getLocalServerUserId(),
                        relationshipDef.getGUID(),
                        super.getPropertiesForInstance(relationshipDef.getPropertiesDefinition()),
                        proxy1GUID,
                        proxy2GUID,
                ...
                assertCondition((pRelationship != null),
                        assertion34,
            ...
            if (pRelationship != null) {
                ...
                /*
                 * Delete and purge the relationship
                 */

                try {

                    metadataCollection.deleteRelationship(workPad.getLocalServerUserId(),
                            pRelationship.getType().getTypeDefGUID(),
                            pRelationship.getType().getTypeDefName(),
                            pRelationship.getGUID());
