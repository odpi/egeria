...
public class TestEntitySearch extends OpenMetadataPerformanceTestCase
{
    ...
    private static final String TEST_CASE_ID   = "repository-entity-search-performance";
    private static final String TEST_CASE_NAME = "Repository entity search performance test case";

    private static final String A_FIND_ENTITIES_ALL     = TEST_CASE_ID + "-findEntities-all";
    private static final String A_FIND_ENTITIES_ALL_MSG = "Repository performs search of all instances, sorting by GUID and paging all results, of type: ";
     ...
     */
    public TestEntitySearch(PerformanceWorkPad workPad,
                            EntityDef          entityDef)
    {
        super(workPad, PerformanceProfile.ENTITY_SEARCH.getProfileId());
    ...
    protected void run() throws Exception
    {
        ...
        super.setSuccessMessage("Entity search performance tests complete for: " + testTypeName);
    }
    ...
    private void findEntities(OMRSMetadataCollection metadataCollection) throws Exception
    {
        ...
        try {
            ...
            while (results != null && !results.isEmpty()) {
                // Only count those entities whose type is precisely the one we searched (to avoid double-counting
                ...
                for (EntityDetail result : results) {
                    if (result.getType().getTypeDefName().equals(entityDef.getName())) {
                        performanceWorkPad.incrementEntitiesFound(1);
                    }
                }
