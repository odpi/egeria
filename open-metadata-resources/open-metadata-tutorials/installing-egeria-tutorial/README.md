...
$ ls
ls
archive-tmp					egeria-2.11-sources.tar.gz
egeria-2.11-distribution		maven-archiver
egeria-2.11-distribution.tar.gz		open-metadata-assemblies-2.11.jar
egeria-2.11-omag-server			rat.txt
egeria-2.11-omag-server.tar.gz
```
...
$ cd ../../../../egeria-install
$ ls
egeria-2.11-distribution.tar.gz
```

It is now possible to unpack the tar file.

```bash
$ tar -xf egeria-2.11-distribution.tar.gz
$ ls
egeria-2.11-distribution.tar.gz	egeria-omag-2.11
```

A new directory is created called **egeria-omag-2.11**.  Change to this
...
$ resources				server-chassis-spring-2.11.jar
```
