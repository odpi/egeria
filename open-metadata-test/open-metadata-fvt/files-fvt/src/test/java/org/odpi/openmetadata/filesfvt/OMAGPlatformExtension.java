...
/**
 * OMAGPlatformExtension stands up the whole deployment that the Files connectors are designed to run
 * in, in one JVM: an OMAG Server Platform and, on it, the four servers a curator would need in order to
 * catalogue and survey a directory tree.
 * <ul>
 *     <li><b>{@value #METADATA_STORE_NAME}</b> - a metadata access store with a PostgreSQL local repository,
 *     the open metadata types and the Core and Files content packs loaded, and its access services
 ...
 *     Files content pack defines: {@code FileSurvey} and {@code FileGovernance}.</li>
 *     <li><b>{@value #INTEGRATION_DAEMON_NAME}</b> - an integration daemon running the content pack's
 *     {@code FilesIntegrationGroup}, which is where the folder cataloguers live.</li>
...
public class OMAGPlatformExtension implements BeforeAllCallback, ExtensionContext.Store.CloseableResource
{
    ...
    /**
     * The user this suite configures and drives every server as.  {@link FilesFvtSecurityConfig} also names
     * the platform's anonymous principal after it, so the calls this suite makes into the view server arrive
     * as this user too.
     */
    public static final String USER_ID = "filesfvtuser";

    /**
     * The metadata access store: PostgreSQL local repository, all access services with Kafka out topics, and
     * the open metadata types plus the Core and Files content packs loaded at start-up.
     */
    public static final String METADATA_STORE_NAME = "filesFvtMetadataStore";

    /**
     * The engine host running the Files content pack's survey and governance engines.
     */
    public static final String ENGINE_HOST_NAME = "filesFvtEngineHost";

    /**
     * The integration daemon running the Files content pack's integration group.
     */
    public static final String INTEGRATION_DAEMON_NAME = "filesFvtIntegrationDaemon";

    /**
     * The view server running the Automated Curation OMVS.
     */
    public static final String VIEW_SERVER_NAME = "filesFvtViewServer";
     ...
     * <br>
     * This is the shortest list that makes the Files content pack work, and each entry earns its place.
     * The types have to be there before any instance can be created.  The Core content pack supplies the
     * generic governance services the Files processes call - create an asset from a template, delete it
     * again, add it to an integration connector as a catalog target - and the Stewardship engine that writes
     * a survey report out as markdown.  The Files content pack itself supplies everything this suite is here
     * to test: the file and folder survey services, the folder cataloguers, and the twenty-odd catalog
     * templates for the file types Egeria recognises.
     * <br>
     ...
     * Files one.  The survey processes end by writing the survey report out as a markdown document, and that
     * step is addressed to the Stewardship engine.  Without it the earlier steps would run, the survey report
     * would be produced, and the process would then sit at its last step for ever - which reads exactly like
     * a broken survey.
     */
                                       ...
                                       long   defaultValue)
    {
        ...
        if (value != null)
        {
            ...
            {
                System.out.println("files-fvt: " + propertyName + " is not a number ('" + value + "') - using " + defaultValue);
            }
        }
     ...
     * {@code -PrunFilesFvtNoKafka}, the access services are configured without out topics and there is no
    ...
    {
        return ! "none".equalsIgnoreCase(getProperty("files.fvt.event.bus", "kafka"));
    }
    ...
    public void beforeAll(ExtensionContext context) throws Exception
    {
        synchronized (OMAGPlatformExtension.class)
        {
            ...
            {
                throw new IllegalStateException("The files-fvt environment failed to start: " + startupFailure.getMessage(),
                                                startupFailure);
            }
            ...
            try
            {
                 ...
                 */
                FilesFvtTestSupport.cleanUpLeftoverTestElements();

                /*
                 * Prepare the directory tree under test.  This needs no server at all, so it happens
                 * here rather than after the governance servers are up.
                 */
                FilesFvtTestSupport.prepareTreeUnderTest();
    ...
    {
        PlatformServicesClient platformServicesClient = new PlatformServicesClient("files-fvt Platform",
                                                                                   platformURLRoot,
                                                                                   null,
                                                                                   null,
                                                                                   null,
    ...
    private void configureMetadataStore() throws Exception
    {
        ...
        configurationClient.setBasicServerProperties("Egeria files-fvt",
                                                     "Metadata access store holding the metadata that the files-fvt suite creates and reads.",
                                                     USER_ID,
                                                     "org.odpi.openmetadata.adapters.connectors.secretsstore.yaml.YAMLSecretsStoreProvider",
                                                     "build/files-fvt-data/secrets.omsecrets",
                                                     "files-fvt",
                                                     platformURLRoot,
        ...
        else
        {
            ...
            System.out.println("files-fvt: running with no event bus - the access services are configured without out topics,"
    ...
    /**
     * Configure the engine host with the two governance engines the Files content pack defines.
     * <br>
    ...
    private void configureEngineHost() throws Exception
    {
        ...
        configurationClient.setServerUserId(USER_ID);
        configurationClient.setBasicServerProperties("Egeria files-fvt",
                                                     "Engine host running the Files content pack's survey and governance engines.",
                                                     USER_ID,
                                                     "org.odpi.openmetadata.adapters.connectors.secretsstore.yaml.YAMLSecretsStoreProvider",
                                                     "build/files-fvt-data/secrets.omsecrets",
                                                     "files-fvt",
    ...
    /**
     * Configure the integration daemon with the Files content pack's integration group.
     * <br>
    ...
    private void configureIntegrationDaemon() throws Exception
    {
        ...
        configurationClient.setServerUserId(USER_ID);
        configurationClient.setBasicServerProperties("Egeria files-fvt",
                                                     "Integration daemon running the Files content pack's integration group.",
                                                     USER_ID,
                                                     "org.odpi.openmetadata.adapters.connectors.secretsstore.yaml.YAMLSecretsStoreProvider",
                                                     "build/files-fvt-data/secrets.omsecrets",
                                                     "files-fvt",
    ...
    private void configureViewServer() throws Exception
    {
        ...
        configurationClient.setServerUserId(USER_ID);
        configurationClient.setBasicServerProperties("Egeria files-fvt",
                                                     "View server running the Automated Curation OMVS for the files-fvt suite.",
                                                     USER_ID,
                                                     "org.odpi.openmetadata.adapters.connectors.secretsstore.yaml.YAMLSecretsStoreProvider",
                                                     "build/files-fvt-data/secrets.omsecrets",
                                                     "files-fvt",
                             ...
                             String                 serverName) throws Exception
    {
        ...
        System.out.println("files-fvt: started " + serverName);
    }
    ...
    private void waitForGovernanceEngines() throws Exception
    {
        ...
        long timeoutMilliseconds = getLongProperty("files.fvt.governance.server.timeout.seconds", 180) * 1000;
        long pollMilliseconds    = getLongProperty("files.fvt.governance.server.poll.seconds", 2) * 1000;
        ...
        while (System.currentTimeMillis() < giveUpTime)
        {
            ...
            if (summaries != null)
            {
                ...
                {
                    System.out.println("files-fvt: governance engines running -" + statuses);
                    return;
                }
            }
        ...
        throw new IllegalStateException("The governance engines on " + ENGINE_HOST_NAME + " did not reach RUNNING within "
                                                ...
                                                + METADATA_STORE_NAME + " - check that the Files content pack loaded.");
    }
    ...
    {
        String secretsStoreLocation = getProperty("files.fvt.server.secrets.store", null);
        String secretsCollection    = getProperty("files.fvt.server.secrets.collection", null);
        ...
        {
            System.out.println("files-fvt: could not read '" + secretName + "' from secrets collection " + secretsCollection
    ...
    public void close()
    {
        if (platformContext != null)
        {
            ...
            {
                System.out.println("Problem shutting down the files-fvt platform: " + error.getMessage());
            }
            finally
            {
