...
package org.odpi.openmetadata.fvt.opentypes.entities.Port;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import org.odpi.openmetadata.repositoryservices.connectors.stores.metadatacollectionstore.properties.instances.Relationship;

import  org.odpi.openmetadata.fvt.opentypes.references.PortToProcess.ProcessReference;
...
import  org.odpi.openmetadata.fvt.opentypes.references.PortToPort.SubPortReference;
...
import  org.odpi.openmetadata.fvt.opentypes.references.PortToPort.ConsumingPortReference;
import org.odpi.openmetadata.fvt.opentypes.relationships.ParentPort.ParentPort;
import org.odpi.openmetadata.fvt.opentypes.relationships.ParentPort.ParentPortMapper;
import  org.odpi.openmetadata.fvt.opentypes.references.PortToPort.SecondPortReference;
...
import  org.odpi.openmetadata.fvt.opentypes.references.PortToPort.FirstPortReference;
import org.odpi.openmetadata.fvt.opentypes.relationships.PortWire.PortWire;
import org.odpi.openmetadata.fvt.opentypes.relationships.PortWire.PortWireMapper;
import  org.odpi.openmetadata.fvt.opentypes.references.PortToDeployedAPI.DeployedAPIReference;
...
/**
 * References for entity  Port. References are relationships represented as an attribute. Exposing relationships information
 * as references can make the relationships information more digestable.
 */
@JsonAutoDetect(getterVisibility=PUBLIC_ONLY, setterVisibility=PUBLIC_ONLY, fieldVisibility=NONE)
@JsonInclude(JsonInclude.Include.NON_NULL)
@JsonIgnoreProperties(ignoreUnknown=true)
public class PortReferences implements Serializable {
    private static final Logger log = LoggerFactory.getLogger(PortReferences.class);
    private static final String className = PortReferences.class.getName();
    ...
    public static final String[] REFERENCE_NAMES_SET_VALUES = new String[] {
     ...
     /**
       * We are passed a set of omrs relationships that are associated with a entity Port
       * Each of these relationships should map to a reference (a uniquely named attribute in Port).
       *
       ...
       */
     public PortReferences(String entityGuid, Set<Line> lines)  {
         ...
         for (Line relationships: lines) {
            for (int i=0;i< RELATIONSHIP_NAMES_SET_VALUES.length; i++) {
               if (relationships.getName().equals(RELATIONSHIP_NAMES_SET_VALUES[i])) {
                    ...
                    if ("process".equals(referenceName)) {
                         ProcessPort processPort_relationship = (ProcessPort)relationships;
                         ProcessReference processReference = new ProcessReference(entityGuid,processPort_relationship);
                         if ( process== null ) {
                              process = new HashSet();
                         }
                          process.add(processReference);
                    }
                    ...
                    if ("subPort".equals(referenceName)) {
                         ParentPort parentPort_relationship = (ParentPort)relationships;
                         subPort = new SubPortReference(entityGuid, parentPort_relationship);
                    }
                    if ("consumingPort".equals(referenceName)) {
                         ParentPort parentPort_relationship = (ParentPort)relationships;
                         consumingPort = new ConsumingPortReference(entityGuid, parentPort_relationship);
                    }
                    if ("secondPort".equals(referenceName)) {
                         PortWire portWire_relationship = (PortWire)relationships;
                         secondPort = new SecondPortReference(entityGuid, portWire_relationship);
                    }
                    if ("firstPort".equals(referenceName)) {
                         PortWire portWire_relationship = (PortWire)relationships;
                         firstPort = new FirstPortReference(entityGuid, portWire_relationship);
                    }
                    if ("deployedAPI".equals(referenceName)) {
                         PortInterface portInterface_relationship = (PortInterface)relationships;
                         deployedAPI = new DeployedAPIReference(entityGuid, portInterface_relationship);
                    }
...
// Singular properties
    private SubPortReference subPort;
    private ConsumingPortReference consumingPort;
    private SecondPortReference secondPort;
    private FirstPortReference firstPort;
    private DeployedAPIReference deployedAPI;
    private RelatedToObjectAnnotationsReference relatedToObjectAnnotations;
// Set properties

    private Set<ProcessReference> process;
...
// List properties
    ...
    // setters and setters
    public SubPortReference getSubPortReference() {
        return subPort;   }

    public void setSubPortReference(SubPortReference subPort) {
        this.subPort = subPort; }
    public ConsumingPortReference getConsumingPortReference() {
        return consumingPort;   }

    public void setConsumingPortReference(ConsumingPortReference consumingPort) {
        this.consumingPort = consumingPort; }
    public SecondPortReference getSecondPortReference() {
        return secondPort;   }

    public void setSecondPortReference(SecondPortReference secondPort) {
        this.secondPort = secondPort; }
    public FirstPortReference getFirstPortReference() {
        return firstPort;   }

    public void setFirstPortReference(FirstPortReference firstPort) {
        this.firstPort = firstPort; }
    public DeployedAPIReference getDeployedAPIReference() {
        return deployedAPI;   }

    public void setDeployedAPIReference(DeployedAPIReference deployedAPI) {
...
// Sets
    public Set<ProcessReference> getProcessReferences() {
        return process;
    }

    public void setProcessReferences(Set<ProcessReference> process) {
...
// Lists
 ...
 public StringBuilder toString(StringBuilder sb) {
        ...
        sb.append("PortReferences{");
        sb.append("processReference='").append(process.toString());
        ...
        sb.append("subPortReference='").append(subPort.toString());
        sb.append("consumingPortReference='").append(consumingPort.toString());
        sb.append("secondPortReference='").append(secondPort.toString());
        sb.append("firstPortReference='").append(firstPort.toString());
        sb.append("deployedAPIReference='").append(deployedAPI.toString());
    ...
    public boolean equals(Object o) {
        ...
        if (!super.equals(o)) { return false; }
         PortReferences typedThat = (PortReferences) o;
      // compare single cardinality attributes
         if (this.process != null && !Objects.equals(this.process,typedThat.process)) {
                            return false;
                 }
         if (this.subPort != null && !Objects.equals(this.subPort,typedThat.subPort)) {
                            return false;
                 }
         if (this.consumingPort != null && !Objects.equals(this.consumingPort,typedThat.consumingPort)) {
                            return false;
                 }
         if (this.secondPort != null && !Objects.equals(this.secondPort,typedThat.secondPort)) {
                            return false;
                 }
         if (this.firstPort != null && !Objects.equals(this.firstPort,typedThat.firstPort)) {
                            return false;
                 }
         if (this.deployedAPI != null && !Objects.equals(this.deployedAPI,typedThat.deployedAPI)) {
                            return false;
                 }
