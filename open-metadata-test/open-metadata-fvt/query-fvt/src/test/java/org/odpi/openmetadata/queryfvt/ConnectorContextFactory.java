 ...
 * platform - so that the query-fvt tests can drive its connector context clients (the generic
...
class ConnectorContextFactory
{
     ...
     * the query-fvt server, whose ordinary "delete" calls perform a soft delete (the element remains
     * retrievable via a status-filtered query, only vanishing from the default "active only" view) -
     * this is what most of the test suite needs, since {@link QueryFvtStatusFVT} specifically checks
    ...
    /**
     * Create a new connector context backed by a fresh {@link EgeriaOpenMetadataStoreClient} pointed at
     * the query-fvt server, using the supplied delete method for any "delete" call made through it.
     *
