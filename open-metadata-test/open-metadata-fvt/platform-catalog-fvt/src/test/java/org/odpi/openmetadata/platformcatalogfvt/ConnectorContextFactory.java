 ...
 * platform - so that the platform-catalog-fvt tests can drive its connector context clients (the generic
 ...
 * <br>
 * This is deliberately the same route the cataloguer itself takes to open metadata, so a test that reads
 * the repository sees exactly what the connector would see - including the deduplication, which is applied
 * on the way out of the repository handler rather than by any client.
 */
class ConnectorContextFactory
{
    static final int MAX_PAGE_SIZE = PlatformCatalogFvtTestSupport.MAX_PAGE_SIZE;


    /**
     * Create a new connector context pointed at the platform-catalog-fvt metadata access store.
     *
    ...
    /**
     * Create a new connector context pointed at a named metadata access store on a named platform.  This is
     * how {@link PlatformCatalogCohortFVT} reads the second platform's repository directly, rather than
     * through the federated view the first one gives it.
     *
     * @param serverName metadata access store to talk to
     * @param platformURLRoot platform it is running on
     * @return connector context
     * @throws Exception problem creating the underlying client
     */
    static ConnectorContextBase newContext(String serverName,
                                           String platformURLRoot) throws Exception
    {
        OpenMetadataClient openMetadataClient = new EgeriaOpenMetadataStoreClient(serverName,
                                                                                 platformURLRoot,
                                                                                 (String) null,
                                                                                 null,
                                                                                 null,
