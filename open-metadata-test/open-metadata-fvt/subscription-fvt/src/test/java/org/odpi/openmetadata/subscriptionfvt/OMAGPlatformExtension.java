...
/**
 * OMAGPlatformExtension stands up the whole deployment that a digital product catalogue needs, in one JVM: an
 * OMAG Server Platform and, on it, the three servers that stand between a consumer and a subscription.
 * <ul>
 *     <li><b>{@value #METADATA_STORE_NAME}</b> - a metadata access store with a PostgreSQL local repository,
 *     the open metadata types and the Core, PostgreSQL and Open Metadata Digital Products content packs
 *     loaded, and its access services publishing to a real Apache Kafka broker.</li>
 *     <li><b>{@value #INTEGRATION_DAEMON_NAME}</b> - an integration daemon running the products content
 *     pack's {@code Egeria:IntegrationGroup:Jacquard}, which is where the Jacquard Digital Product Loom
 *     runs.  Jacquard is what builds the catalogue: nothing in this suite creates a product by hand.</li>
 *     <li><b>{@value #ENGINE_HOST_NAME}</b> - an engine host running the Egeria Governance and Egeria
 *     Watchdog engines, which is where the create-subscription governance service and the Baudot
 *     subscription manager run.</li>
 * </ul>
 * There is no view server.  The Product Catalog OMVS - the service whose description is "supports searching a
 * digital product catalogue and subscribing to specific products" - currently publishes no endpoints at all,
 * so a consumer's journey cannot yet be driven through it.  This suite drives the same journey through the
 * clients underneath instead, and that gap is worth knowing about when reading these tests: they are testing
 * that the catalogue <em>can</em> be navigated, not that anybody can navigate it through a supported API.
 * <br>
 ...
 * the access services' out topics.  With no broker, all three servers still start and the repository still
 ...
 * where the other two read their configuration from.  After they are started this class waits for their
 * engines and integration groups to reach {@code RUNNING} rather than assuming that an activated server is a
 * ready one - both are populated from the metadata access store <em>after</em> activation returns, and a
...
public class OMAGPlatformExtension implements BeforeAllCallback, ExtensionContext.Store.CloseableResource
{
     ...
     * as this user too.
     */
    public static final String USER_ID = "subscriptionfvtuser";
     ...
     */
    public static final String METADATA_STORE_NAME = "subscriptionFvtMetadataStore";

    /**
     * The engine host running the PostgreSQL content pack's survey and governance engines.
     */
    public static final String ENGINE_HOST_NAME = "subscriptionFvtEngineHost";

    /**
     * The integration daemon running the PostgreSQL content pack's integration group.
     */
    public static final String INTEGRATION_DAEMON_NAME = "subscriptionFvtIntegrationDaemon";
     ...
     */
    private static final String METADATA_COLLECTION_ID = "3f7a1c25-9d64-4a18-bc03-73756273637221";
     ...
     * daemon and engine host machinery the other two packs plug into.  The PostgreSQL pack supplies the two
                                       ...
                                       long   defaultValue)
    {
        ...
        if (value != null)
        {
            ...
            {
                System.out.println("subscription-fvt: " + propertyName + " is not a number ('" + value + "') - using " + defaultValue);
            }
        }
    ...
    {
        return ! "none".equalsIgnoreCase(getProperty("subscription.fvt.event.bus", "kafka"));
    }
    ...
    public void beforeAll(ExtensionContext context) throws Exception
    {
        synchronized (OMAGPlatformExtension.class)
        {
            ...
            {
                throw new IllegalStateException("The subscription-fvt environment failed to start: " + startupFailure.getMessage(),
                                                startupFailure);
            }
            ...
            try
            {
                 ...
                 */
                SubscriptionFvtTestSupport.cleanUpLeftoverTestElements();

                /*
                 * The product catalogue goes too, so that this run's Jacquard builds its own rather than
                 * reusing the one an earlier run - and an earlier version of Jacquard - left behind.
                 */
                SubscriptionFvtTestSupport.clearCatalogue();
                 ...
                 */
                SubscriptionFvtTestSupport.prepareServerUnderTest();
    ...
    {
        PlatformServicesClient platformServicesClient = new PlatformServicesClient("subscription-fvt Platform",
                                                                                   platformURLRoot,
                                                                                   null,
                                                                                   null,
                                                                                   null,
    ...
    private void configureMetadataStore() throws Exception
    {
        ...
        configurationClient.setBasicServerProperties("Egeria subscription-fvt",
                                                     "Metadata access store holding the metadata that the subscription-fvt suite creates and reads.",
                                                     USER_ID,
                                                     "org.odpi.openmetadata.adapters.connectors.secretsstore.yaml.YAMLSecretsStoreProvider",
                                                     "build/subscription-fvt-data/secrets.omsecrets",
                                                     "subscription-fvt",
                                                     platformURLRoot,
        ...
        else
        {
            ...
            System.out.println("subscription-fvt: running with no event bus - the access services are configured without out topics,"
                             ...
                             String                 serverName) throws Exception
    {
        ...
        System.out.println("subscription-fvt: started " + serverName);
    }
    ...
    private void waitForGovernanceEngines() throws Exception
    {
        ...
        long timeoutMilliseconds = getLongProperty("subscription.fvt.governance.server.timeout.seconds", 180) * 1000;
        long pollMilliseconds    = getLongProperty("subscription.fvt.governance.server.poll.seconds", 2) * 1000;
        ...
        while (System.currentTimeMillis() < giveUpTime)
        {
            ...
            if (summaries != null)
            {
                ...
                {
                    System.out.println("subscription-fvt: governance engines running -" + statuses);
                    return;
                }
            }
                    ...
                    for (IntegrationConnectorReport connectorReport : connectorReports)
                    {
                        if (connectorReport.getFailingExceptionMessage() != null)
                        {
                            throw new IllegalStateException("Integration connector " + connectorReport.getConnectorName() + " in group "
                                                                    ...
                                                                    + " missing from subscription-fvt's test runtime classpath.");
                        }
                    }

                    System.out.println("subscription-fvt: integration group " + INTEGRATION_GROUP.getQualifiedName() + " running with "
    ...
    {
        String secretsStoreLocation = getProperty("subscription.fvt.server.secrets.store", null);
        String secretsCollection    = getProperty("subscription.fvt.server.secrets.collection", null);
        ...
        {
            System.out.println("subscription-fvt: could not read '" + secretName + "' from secrets collection " + secretsCollection
    ...
    public void close()
    {
        if (platformContext != null)
        {
            ...
            {
                System.out.println("Problem shutting down the subscription-fvt platform: " + error.getMessage());
            }
            finally
            {
