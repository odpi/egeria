...
class EngineActionWaiter
{
    ...
    EngineActionWaiter() throws Exception
    {
        ...
        this.timeoutMilliseconds = OMAGPlatformExtension.getLongProperty("subscription.fvt.engine.action.timeout.seconds", 300) * 1000;
        this.pollMilliseconds    = OMAGPlatformExtension.getLongProperty("subscription.fvt.engine.action.poll.seconds", 2) * 1000;
    }
                                       ...
                                       EngineActionElement engineAction)
    {
        ...
        if ((status == ActivityStatus.REQUESTED) || (status == ActivityStatus.APPROVED) || (status == ActivityStatus.WAITING))
        {
            ...
            return preamble + " - it is still " + status + ", so no engine host claimed it."
                           ...
                           + " wait out that window.  Raise subscription.fvt.engine.action.timeout.seconds past ten minutes"
        ...
        return preamble + " - it reached " + status + " and was still there, so the governance service behind request type '"
                       ...
                       + " build/subscription-fvt-data/logs/audit.log.";
    }
}
