...
/**
 * OMAGPlatformExtension starts a single OMAG Server Platform in-process for the whole tabular-data-fvt run,
...
public class OMAGPlatformExtension implements BeforeAllCallback, ExtensionContext.Store.CloseableResource
{
    ...
    /**
     * Name of the metadata access store server that is configured and started for the tabular-data-fvt suite.
     */
    public static final String SERVER_NAME = "tabularDataFvtMetadataStore";

    /**
     * UserId used for all admin, platform and metadata calls made by the tabular-data-fvt suite.
     */
    public static final String USER_ID = "tabulardatafvtuser";
     ...
     */
    /**
     * The page size the tabular data set connectors are configured with.
     * <br><br>
     * This has to be no larger than the server's own maximum, which this suite's server is configured with
     * below.  A connector handed a bigger one does not quietly get fewer rows - the server refuses the
     * request outright, and the connector fails while filling its cache.
     */
        ...
        if (! started)
        {
            synchronized (OMAGPlatformExtension.class)
            {
                if (! started)
                {
                    ...
                    /*
                     * No clean-up step here, unlike client-fvt: this suite only reads.  Nothing it does
                     * leaves anything behind for a later run to trip over.
                     */
                    started = true;
     ...
     *
     * @throws Exception any problem configuring or starting the server is fatal to the whole tabular-data-fvt run
     */
    private void configureAndStartServer() throws Exception
    {
        PlatformServicesClient platformServicesClient = new PlatformServicesClient("tabular-data-fvt Platform",
                                                                                    platformURLRoot,
                                                                                    null,
                                                                                    null,
                                                                                    null,
        ...
        configurationClient.setBasicServerProperties("Egeria tabular-data-fvt",
                                                       "Server used by the tabular-data-fvt functional verification test suite",
                                                       USER_ID,
                                                       "org.odpi.openmetadata.adapters.connectors.secretsstore.yaml.YAMLSecretsStoreProvider",
                                                       "build/tabular-data-fvt-data/secrets.omsecrets",
                                                       "tabular-data-fvt",
                                                       platformURLRoot,
                                                       100);
        ...
        /*
         * No event bus, and the access services are configured without their out topics.  This suite reads
         * open metadata through the tabular data set connectors and asserts on the shape of what they return;
         * nothing in it consumes an event, so a broker would be a dependency that bought the suite nothing
         * and stopped it running on a machine without Kafka.
         */
    ...
    public void close()
    {
        if (platformContext != null)
        {
            try
            {
                new PlatformServicesClient("tabular-data-fvt Platform", platformURLRoot, null, null, null, USER_ID, null)
