...
/**
 * TestAssetCommentReplies tests the invocation of a specific property iterator.
 */
public class TestAssetCommentReplies
{
    /**
     ...
     */
    private AssetCommentReplies  getPropertyIterator(int    totalElementCount, int    maxCacheSize)
    {
        return new MockAssetCommentReplies(null, totalElementCount, maxCacheSize);
    }
    ...
    {
        AssetCommentReplies iterator = getPropertyIterator(totalElementCount, maxCacheSize);
    ...
    {
        AssetCommentReplies propertyIterator = getPropertyIterator(10, 100);

        propertyIterator.next();

        AssetCommentReplies propertyIteratorClone = new MockAssetCommentReplies(null, propertyIterator);
    ...
    {
        AssetCommentReplies  propertyIterator = getPropertyIterator(30, 10);

        assertTrue(propertyIterator.getElementCount() == 30);

        AssetCommentReplies  clonedPropertyIterator = new MockAssetCommentReplies(null, propertyIterator);

        assertTrue(clonedPropertyIterator.getElementCount() == 30);

        clonedPropertyIterator = new MockAssetCommentReplies(null, null);
    ...
    {
        assertTrue(getPropertyIterator(10, 100).toString().contains("AssetCommentReplies"));
    }
}