...
/**
 * TestAssetConnections tests the invocation of a specific property iterator.
 */
public class TestAssetConnections
{
    /**
     ...
     */
    private AssetConnections  getPropertyIterator(int    totalElementCount, int    maxCacheSize)
    {
        return new MockAssetConnections(null, totalElementCount, maxCacheSize);
    }
    ...
    {
        AssetConnections iterator = getPropertyIterator(totalElementCount, maxCacheSize);
    ...
    {
        AssetConnections propertyIterator = getPropertyIterator(10, 100);

        propertyIterator.next();

        AssetConnections propertyIteratorClone = new MockAssetConnections(null, propertyIterator);
    ...
    {
        AssetConnections  propertyIterator = getPropertyIterator(30, 10);

        assertTrue(propertyIterator.getElementCount() == 30);

        AssetConnections  clonedPropertyIterator = new MockAssetConnections(null, propertyIterator);

        assertTrue(clonedPropertyIterator.getElementCount() == 30);

        clonedPropertyIterator = new MockAssetConnections(null, null);
    ...
    {
        assertTrue(getPropertyIterator(10, 100).toString().contains("AssetConnections"));
    }
}