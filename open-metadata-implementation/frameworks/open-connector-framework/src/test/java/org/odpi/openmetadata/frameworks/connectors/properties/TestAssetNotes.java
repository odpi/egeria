...
/**
 * TestAssetNotes tests the invocation of a specific property iterator.
 */
public class TestAssetNotes
{
    /**
     ...
     */
    private AssetNotes  getPropertyIterator(int    totalElementCount, int    maxCacheSize)
    {
        return new MockAssetNotes(null, totalElementCount, maxCacheSize);
    }
    ...
    {
        AssetNotes iterator = getPropertyIterator(totalElementCount, maxCacheSize);
    ...
    {
        AssetNotes propertyIterator = getPropertyIterator(10, 100);

        propertyIterator.next();

        AssetNotes propertyIteratorClone = new MockAssetNotes(null, propertyIterator);
    ...
    {
        AssetNotes  propertyIterator = getPropertyIterator(30, 10);

        assertTrue(propertyIterator.getElementCount() == 30);

        AssetNotes  clonedPropertyIterator = new MockAssetNotes(null, propertyIterator);

        assertTrue(clonedPropertyIterator.getElementCount() == 30);

        clonedPropertyIterator = new MockAssetNotes(null, null);
    ...
    {
        assertTrue(getPropertyIterator(10, 100).toString().contains("AssetNotes"));
    }
}