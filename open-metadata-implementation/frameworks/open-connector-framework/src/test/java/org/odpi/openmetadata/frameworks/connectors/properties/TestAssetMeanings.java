...
/**
 * TestAssetMeanings tests the invocation of a specific property iterator.
 */
public class TestAssetMeanings
{
    /**
     ...
     */
    private AssetMeanings  getPropertyIterator(int    totalElementCount,
                                               int    maxCacheSize)
    {
        return new MockAssetMeanings(null,
                                     totalElementCount,
                                     maxCacheSize);
    }
     ...
     * @param maxCacheSize size of cache
     */
    private void  validatePropertyIterator(int    totalElementCount,
                                           int    maxCacheSize)
    {
        AssetMeanings iterator = getPropertyIterator(totalElementCount, maxCacheSize);
    ...
    {
        AssetMeanings propertyIterator = getPropertyIterator(10,
                                                             100);

        propertyIterator.next();

        AssetMeanings propertyIteratorClone = new MockAssetMeanings(null,
                                                                    propertyIterator);
    ...
    {
        AssetMeanings  propertyIterator = getPropertyIterator(30, 10);

        assertTrue(propertyIterator.getElementCount() == 30);

        AssetMeanings  clonedPropertyIterator = new MockAssetMeanings(null, propertyIterator);

        assertTrue(clonedPropertyIterator.getElementCount() == 30);

        clonedPropertyIterator = new MockAssetMeanings(null, null);
    ...
    {
        assertTrue(getPropertyIterator(10, 100).toString().contains("AssetMeanings"));
    }
}