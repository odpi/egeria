...
/**
 * TestAssetLocations tests the invocation of a specific property iterator.
 */
public class TestAssetLocations
{
     ...
     */
    private AssetLocations  getPropertyIterator(int    totalElementCount, int    maxCacheSize)
    {
        return new MockAssetLocations(null, totalElementCount, maxCacheSize);
    }
    ...
    {
        AssetLocations iterator = getPropertyIterator(totalElementCount, maxCacheSize);
    ...
    {
        AssetLocations propertyIterator = getPropertyIterator(10, 100);

        propertyIterator.next();

        AssetLocations propertyIteratorClone = new MockAssetLocations(null, propertyIterator);
    ...
    {
        AssetLocations  propertyIterator = getPropertyIterator(30, 10);

        assertTrue(propertyIterator.getElementCount() == 30);

        AssetLocations  clonedPropertyIterator = new MockAssetLocations(null, propertyIterator);

        assertTrue(clonedPropertyIterator.getElementCount() == 30);

        clonedPropertyIterator = new MockAssetLocations(null, null);
    ...
    {
        assertTrue(getPropertyIterator(10, 100).toString().contains("AssetLocations"));
    }
}