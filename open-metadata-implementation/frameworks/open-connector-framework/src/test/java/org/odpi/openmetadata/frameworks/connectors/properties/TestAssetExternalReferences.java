...
/**
 * TestAssetExternalReferences tests the invocation of a specific property iterator.
 */
public class TestAssetExternalReferences
{
    /**
     ...
     */
    private AssetExternalReferences  getPropertyIterator(int    totalElementCount, int    maxCacheSize)
    {
        return new MockAssetExternalReferences(null, totalElementCount, maxCacheSize);
    }
    ...
    {
        AssetExternalReferences iterator = getPropertyIterator(totalElementCount, maxCacheSize);
    ...
    {
        AssetExternalReferences propertyIterator = getPropertyIterator(10, 100);

        propertyIterator.next();

        AssetExternalReferences propertyIteratorClone = new MockAssetExternalReferences(null, propertyIterator);
    ...
    {
        AssetExternalReferences  propertyIterator = getPropertyIterator(30, 10);

        assertTrue(propertyIterator.getElementCount() == 30);

        AssetExternalReferences  clonedPropertyIterator = new MockAssetExternalReferences(null, propertyIterator);

        assertTrue(clonedPropertyIterator.getElementCount() == 30);

        clonedPropertyIterator = new MockAssetExternalReferences(null, null);
    ...
    {
        assertTrue(getPropertyIterator(10, 100).toString().contains("AssetExternalReferences"));
    }
}