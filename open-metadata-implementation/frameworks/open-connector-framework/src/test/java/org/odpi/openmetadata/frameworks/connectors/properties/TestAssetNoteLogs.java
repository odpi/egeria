...
/**
 * TestAssetNoteLogs tests the invocation of a specific property iterator.
 */
public class TestAssetNoteLogs
{
    /**
     ...
     */
    private AssetNoteLogs  getPropertyIterator(int    totalElementCount, int    maxCacheSize)
    {
        return new MockAssetNoteLogs(null, totalElementCount, maxCacheSize);
    }
    ...
    {
        AssetNoteLogs iterator = getPropertyIterator(totalElementCount, maxCacheSize);
    ...
    {
        AssetNoteLogs propertyIterator = getPropertyIterator(10, 100);

        propertyIterator.next();

        AssetNoteLogs propertyIteratorClone = new MockAssetNoteLogs(null, propertyIterator);
    ...
    {
        AssetNoteLogs  propertyIterator = getPropertyIterator(30, 10);

        assertTrue(propertyIterator.getElementCount() == 30);

        AssetNoteLogs  clonedPropertyIterator = new MockAssetNoteLogs(null, propertyIterator);

        assertTrue(clonedPropertyIterator.getElementCount() == 30);

        clonedPropertyIterator = new MockAssetNoteLogs(null, null);
    ...
    {
        assertTrue(getPropertyIterator(10, 100).toString().contains("AssetNoteLogs"));
    }
}