...
/**
 * TestAssetRelatedMediaReferences tests the invocation of a specific property iterator.
 */
public class TestAssetRelatedMediaReferences
{
    /**
     ...
     */
    private AssetRelatedMediaReferences  getPropertyIterator(int    totalElementCount, int    maxCacheSize)
    {
        return new MockAssetRelatedMediaReferences(null, totalElementCount, maxCacheSize);
    }
    ...
    {
        AssetRelatedMediaReferences iterator = getPropertyIterator(totalElementCount, maxCacheSize);
    ...
    {
        AssetRelatedMediaReferences propertyIterator = getPropertyIterator(10, 100);

        propertyIterator.next();

        AssetRelatedMediaReferences propertyIteratorClone = new MockAssetRelatedMediaReferences(null, propertyIterator);
    ...
    {
        AssetRelatedMediaReferences  propertyIterator = getPropertyIterator(30, 10);

        assertTrue(propertyIterator.getElementCount() == 30);

        AssetRelatedMediaReferences  clonedPropertyIterator = new MockAssetRelatedMediaReferences(null, propertyIterator);

        assertTrue(clonedPropertyIterator.getElementCount() == 30);

        clonedPropertyIterator = new MockAssetRelatedMediaReferences(null, null);
    ...
    {
        assertTrue(getPropertyIterator(10, 100).toString().contains("AssetRelatedMediaReferences"));
    }
}