...
/**
 * TestCommentConversationReplies tests the invocation of a specific property iterator.
 */
public class TestCommentConversationReplies
{
    /**
     ...
     */
    private CommentReplies getPropertyIterator(int    totalElementCount, int    maxCacheSize)
    {
        return new MockCommentReplies(totalElementCount, maxCacheSize);
    }
    ...
    {
        CommentReplies iterator = getPropertyIterator(totalElementCount, maxCacheSize);
    ...
    {
        assertTrue(getPropertyIterator(10, 100).toString().contains("CommentReplies"));
    }
}