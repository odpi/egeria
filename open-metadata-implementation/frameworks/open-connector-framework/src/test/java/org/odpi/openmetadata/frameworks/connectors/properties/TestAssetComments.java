...
/**
 * TestAssetComments tests the invocation of a specific property iterator.
 */
public class TestAssetComments
{
    /**
     ...
     */
    private AssetComments  getPropertyIterator(int    totalElementCount, int    maxCacheSize)
    {
        return new MockAssetComments(null, totalElementCount, maxCacheSize);
    }
    ...
    {
        AssetComments iterator = getPropertyIterator(totalElementCount, maxCacheSize);
    ...
    {
        AssetComments propertyIterator = getPropertyIterator(10, 100);

        propertyIterator.next();

        AssetComments propertyIteratorClone = new MockAssetComments(null, propertyIterator);
    ...
    {
        AssetComments  propertyIterator = getPropertyIterator(30, 10);

        assertTrue(propertyIterator.getElementCount() == 30);

        AssetComments  clonedPropertyIterator = new MockAssetComments(null, propertyIterator);

        assertTrue(clonedPropertyIterator.getElementCount() == 30);

        clonedPropertyIterator = new MockAssetComments(null, null);
    ...
    {
        assertTrue(getPropertyIterator(10, 100).toString().contains("AssetComments"));
    }
}