...
/**
 * TestExternalReferences tests the invocation of a specific property iterator.
 */
public class TestExternalReferences
{
     ...
     */
    private ExternalReferences getPropertyIterator(int    totalElementCount, int    maxCacheSize)
    {
        return new MockExternalReferences( totalElementCount, maxCacheSize);
    }
    ...
    {
        ExternalReferences iterator = getPropertyIterator(totalElementCount, maxCacheSize);
    ...
    {
        assertTrue(getPropertyIterator(10, 100).toString().contains("ExternalReferences"));
    }
}