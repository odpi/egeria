/* SPDX-License-Identifier: Apache-2.0 */
/* Copyright Contributors to the ODPi Egeria project. */
package org.odpi.openmetadata.frameworks.integration.bitol.odcs;

/**
 * DataContractComparisonOperator lists the operators an ODCS data quality check uses to compare the measured value with its threshold values.
 * The Bitol standards carry these values as strings, and permit additional values, so the beans hold the
 * string and this enumeration documents the standard values and provides a lookup.
 */
public enum DataContractComparisonOperator
{
    /**
     * The measured value must equal the threshold value.
     */
    MUST_BE                    ("mustBe", "The measured value must equal the threshold value."),

    /**
     * The measured value must not equal the threshold value.
     */
    MUST_NOT_BE                ("mustNotBe", "The measured value must not equal the threshold value."),

    /**
     * The measured value must be greater than the threshold value.
     */
    MUST_BE_GREATER_THAN       ("mustBeGreaterThan", "The measured value must be greater than the threshold value."),

    /**
     * The measured value must be greater than or equal to the threshold value.
     */
    MUST_BE_GREATER_OR_EQUAL_TO("mustBeGreaterOrEqualTo", "The measured value must be greater than or equal to the threshold value."),

    /**
     * The measured value must be less than the threshold value.
     */
    MUST_BE_LESS_THAN          ("mustBeLessThan", "The measured value must be less than the threshold value."),

    /**
     * The measured value must be less than or equal to the threshold value.
     */
    MUST_BE_LESS_OR_EQUAL_TO   ("mustBeLessOrEqualTo", "The measured value must be less than or equal to the threshold value."),

    /**
     * The measured value must lie between the two threshold values.
     */
    MUST_BE_BETWEEN            ("mustBeBetween", "The measured value must lie between the two threshold values."),

    /**
     * The measured value must not lie between the two threshold values.
     */
    MUST_NOT_BE_BETWEEN        ("mustNotBeBetween", "The measured value must not lie between the two threshold values.");

    private final String value;
    private final String description;


    /**
     * Constructor for the enumeration.
     *
     * @param value the string used in the document
     * @param description description of the meaning of the value
     */
    DataContractComparisonOperator(String value,
                          String description)
    {
        this.value       = value;
        this.description = description;
    }


    /**
     * Return the string used in the document.
     *
     * @return string value
     */
    public String getValue()
    {
        return value;
    }


    /**
     * Return the description of the meaning of the value.
     *
     * @return string description
     */
    public String getDescription()
    {
        return description;
    }


    /**
     * Return the enumeration that matches the supplied document value (case-insensitive).
     *
     * @param value string from the document
     * @return matching enumeration or null if the value is not one of the standard values
     */
    public static DataContractComparisonOperator fromValue(String value)
    {
        if (value != null)
        {
            for (DataContractComparisonOperator candidate : DataContractComparisonOperator.values())
            {
                if (candidate.value.equalsIgnoreCase(value))
                {
                    return candidate;
                }
            }
        }

        return null;
    }


    /**
     * Standard toString method.
     *
     * @return print out of variables in a JSON-style
     */
    @Override
    public String toString()
    {
        return "DataContractComparisonOperator{value='" + value + "'}";
    }
}
