...
package org.odpi.openmetadata.viewservices.lineagelinker.admin;
...
import org.odpi.openmetadata.viewservices.lineagelinker.ffdc.LineageLinkerAuditCode;
import org.odpi.openmetadata.viewservices.lineagelinker.server.LineageLinkerInstance;

import java.util.List;

/**
 * LineageLinkerAdmin is the class that is called by the View Server to initialize and terminate
 * the Lineage Linker OMVS.  The initialization call provides this OMVS with the Audit log and configuration.
 */
public class LineageLinkerAdmin extends ViewServerGenericServiceAdmin
{
    private AuditLog              auditLog   = null;
    private LineageLinkerInstance instance   = null;
...

    {
        ...
        auditLog.logMessage(actionDescription, LineageLinkerAuditCode.SERVICE_INITIALIZING.getMessageDefinition());
        ...
        catch (Exception error)
        {
            auditLog.logException(actionDescription,
                                  LineageLinkerAuditCode.SERVICE_INSTANCE_FAILURE.getMessageDefinition(error.getMessage()),
                                  viewServiceConfig.toString(),
                                  error);

            super.throwUnexpectedInitializationException(actionDescription,
                                                         ViewServiceDescription.LINEAGE_LINKER.getViewServiceFullName(),
                                                         error);
        }
    }
    ...
    public void shutdown()
    {
        ...
        auditLog.logMessage(actionDescription, LineageLinkerAuditCode.SERVICE_TERMINATING.getMessageDefinition(serverName));
        ...
        auditLog.logMessage(actionDescription, LineageLinkerAuditCode.SERVICE_SHUTDOWN.getMessageDefinition(serverName));
    }
}