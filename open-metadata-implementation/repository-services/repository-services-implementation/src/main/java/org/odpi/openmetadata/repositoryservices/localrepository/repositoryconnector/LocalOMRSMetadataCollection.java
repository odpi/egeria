...
public class LocalOMRSMetadataCollection extends OMRSMetadataCollectionBase
{
     ...
     * findEntities(), delegating directly to the real metadata collection's countEntities() implementation.
     * Unlike findEntities(), the count returned is not passed through the per-instance security or duplicate
     * processing filtering that is applied to the materialized entity list, since applying that filtering would
     * require fetching every matching entity, which defeats the purpose of an efficient count.
     *
     * @param userId unique identifier for requesting user.
     * @param entityTypeGUID String unique identifier for the entity type of interest (null means any entity type).
     * @param entitySubtypeGUIDs optional list of the unique identifiers (guids) for subtypes of the entityTypeGUID to
     ...
     * semantics as findRelationships(), delegating directly to the real metadata collection's
     * countRelationships() implementation.  Unlike findRelationships(), the count returned is not passed
     * through the per-instance security or duplicate processing filtering that is applied to the materialized
     * relationship list, since applying that filtering would require fetching every matching relationship, which
     * defeats the purpose of an efficient count.
     *
     * @param userId unique identifier for requesting user.
     * @param relationshipTypeGUID unique identifier (guid) for the relationship's type.  Null means all types
     *                             (but may be slow so not recommended).
