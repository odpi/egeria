...
import * as d3  from "d3";
  ...
  export function ls (d) {
    ...
    /*
     * This function path_func2 computes the path for either a reflexive or non-reflexive link.
    ...
    export function path_func2(d) {
      ...
      let curve;
      let lessCurvature = true;

      if (!lessCurvature) {
      if (Math.abs(dy) >= Math.abs(dx)) {
        /* vertical ends */
        curve =  'M' + source.x + ',' + source.y
      ...
      if (Math.abs(dy) >= Math.abs(dx)) {
        /* vertical ends */
        curve =  'M' + source.x + ',' + source.y
               + 'C' + (3 * source.x + target.x)/4  + ',' + (source.y + target.y)/2  
               + ' ' + (source.x + 3 * target.x)/4 + ',' + (source.y + target.y)/2  
               + ' ' + target.x + ',' + target.y
      }
      else {
        /* horizontal ends */
        curve =  'M' + source.x + ',' + source.y
               + 'C' + (source.x + target.x)/2 + ',' + (3 * source.y + target.y)/4
               + ' ' + (source.x + target.x)/2 + ',' + (source.y + 3 * target.y)/4
               + ' ' + target.x + ',' + target.y
      }
    }
    ...
    let mid         = {};
    mid.x           = source.x + dx/2.0;
    mid.y           = source.y + dy/2.0;
 
    returnMap.path = curve;
    returnMap.midpoint = mid;
    return returnMap;
  ...
  export function active_func(d, rad) {

    const path = d3.path();
    path.moveTo(           0, -1.0*rad/2.0);
    path.lineTo(     rad/4.0,      rad/2.0);
    path.lineTo(-1.0*rad/4.0,      rad/2.0);
    path.lineTo(           0, -1.0*rad/2.0);
