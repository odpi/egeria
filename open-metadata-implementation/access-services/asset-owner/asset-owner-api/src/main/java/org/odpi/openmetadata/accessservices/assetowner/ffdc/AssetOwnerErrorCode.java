...
public enum AssetOwnerErrorCode
{
    SERVER_URL_NOT_SPECIFIED(400, "OMAS-ASSET-OWNER-400-001 ",
            ...
            "Ensure a valid OMAS Server URL is passed to the Asset Owner when it is created."),
     ...
     *
     *     AssetOwnerErrorCode   errorCode = AssetOwnerErrorCode.SERVER_URL_NOT_SPECIFIED;
     *
    ...
    {
        log.debug(String.format("<== AssetOwnerErrorCode.getMessage(%s)", Arrays.toString(params)));

        MessageFormat mf = new MessageFormat(errorMessage);
        String result = mf.format(params);

        log.debug(String.format("==> AssetOwnerErrorCode.getMessage(%s): %s", Arrays.toString(params), result));
