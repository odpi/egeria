...
public class DeployedVirtualContainer extends Line {
   ...
   //public java.util.Set<String> propertyNames = new HashSet<>();
    ...
    public DeployedVirtualContainer() {
        super("DeployedVirtualContainer");
        super.entity1Name = "hosts";
        super.entity1Type = "Host";
        super.entity2Name = "hostedContainers";
        super.entity2Type = "VirtualContainer";
    }

    public DeployedVirtualContainer(Relationship omrsRelationship) {
        super(omrsRelationship);

        if (!omrsRelationship.getEntityOnePropertyName().equals("hosts")){
            //error
        }
        if (!omrsRelationship.getEntityTwoPropertyName().equals("hostedContainers")){
            //error
        }
        if (!omrsRelationship.getEntityOneProxy().getType().getTypeDefName().equals("Host")){
            //error
        }
        if (!omrsRelationship.getEntityTwoProxy().getType().getTypeDefName().equals("VirtualContainer")){
            //error
        }
    }
