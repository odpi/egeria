...
package org.odpi.openmetadata.accessservices.subjectarea.generated.relationships.AttachedLike;
...
public class AttachedLike extends Line {
   ...
   //public java.util.Set<String> propertyNames = new HashSet<>();
    ...
    public AttachedLike() {
        super("AttachedLike");
        super.entity1Name = "likedElement";
        super.entity1Type = "Referenceable";
        super.entity2Name = "likes";
        super.entity2Type = "Like";
    }

    public AttachedLike(Relationship omrsRelationship) {
        super(omrsRelationship);

        if (!omrsRelationship.getEntityOnePropertyName().equals("likedElement")){
            //error
        }
        if (!omrsRelationship.getEntityTwoPropertyName().equals("likes")){
            //error
        }
        if (!omrsRelationship.getEntityOneProxy().getType().getTypeDefName().equals("Referenceable")){
            //error
        }
        if (!omrsRelationship.getEntityTwoProxy().getType().getTypeDefName().equals("Like")){
            //error
        }
    }
