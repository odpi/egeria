...
public class StewardshipActionOutTopicPublisher
{
                                              ...
                                              String                                   serverName)
    {
        ...
        {
            outTopicAuditLog.logMessage(actionDescription, StewardshipActionAuditCode.SERVICE_PUBLISHING.getMessageDefinition(outTopicName));
        }
    }
                                   ...
                                   Classification            previousClassification)
    {
        ...
        {
            StewardshipActionOutTopicEvent event = new StewardshipActionOutTopicEvent();
            ...
            catch (Exception error)
            {
                outTopicAuditLog.logException(methodName,
                                              StewardshipActionAuditCode.PROCESS_EVENT_EXCEPTION.getMessageDefinition(event.toString(),
    ...
    public void disconnect()
    {
        try
        {
            ...
            if (outTopicAuditLog != null)
            {
                outTopicAuditLog.logMessage(actionDescription, StewardshipActionAuditCode.PUBLISHING_SHUTDOWN.getMessageDefinition(outTopicName));
            }
        }
        catch (Exception error)
        {
            if (outTopicAuditLog != null)
            {
                outTopicAuditLog.logException(actionDescription,
                                              StewardshipActionAuditCode.PUBLISHING_SHUTDOWN_ERROR.getMessageDefinition(error.getClass().getName(),
                                                                                                                      outTopicName,
                                                                                                                      error.getMessage()),
                                              error);
            }
