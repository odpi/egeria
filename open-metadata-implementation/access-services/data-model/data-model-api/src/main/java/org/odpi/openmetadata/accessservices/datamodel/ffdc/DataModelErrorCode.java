...
package org.odpi.openmetadata.accessservices.datamodel.ffdc;
...
/**
 * The DataModelErrorCode is used to define first failure data capture (FFDC) for errors that occur when working with
 * the Data Model OMAS Services.  It is used in conjunction with both Checked and Runtime (unchecked) exceptions.
 *
 ...
 * </ul>
 */
public enum DataModelErrorCode
{
    BAD_OUT_TOPIC_CONNECTION(400, "OMAS-DATA-MODEL-400-001 ",
             "The Data Model Open Metadata Access Service (OMAS) has been passed an invalid connection for publishing events.  The connection was {0}.  The resulting exception of {1} included the following message: {2}",
    ...
    OMRS_NOT_INITIALIZED(404, "OMAS-DATA-MODEL-404-002 ",
                         "The open metadata repository services are not initialized for the {0} operation",
                         "The system is unable to connect to an open metadata repository.",
                         "Check that the server where the Data Model OMAS is running initialized correctly.  " +
                                 "Correct any errors discovered and retry the request when the open metadata services are available."),
    CONNECTION_NOT_FOUND(404, "OMAS-DATA-MODEL-404-005 ",
    ...
    MULTIPLE_ASSETS_FOUND(404, "OMAS-DATA-MODEL-404-008 ",
    ...
    UNKNOWN_ASSET(404, "OMAS-DATA-MODEL-404-009 ",
    ...
    private static final Logger log = LoggerFactory.getLogger(DataModelErrorCode.class);


    /**
     * The constructor for DataModelErrorCode expects to be passed one of the enumeration rows defined in
     * DataModelErrorCode above.   For example:
     *
     *     DataModelErrorCode   errorCode = DataModelErrorCode.ASSET_NOT_FOUND;
     *
     ...
     */
    DataModelErrorCode(int  newHTTPErrorCode, String newErrorMessageId, String newErrorMessage, String newSystemAction, String newUserAction)
    {
    ...
    {
        log.debug(String.format("<== DataModelErrorCode.getMessage(%s)", Arrays.toString(params)));

        MessageFormat mf = new MessageFormat(errorMessage);
        String result = mf.format(params);

        log.debug(String.format("==> DataModelErrorCode.getMessage(%s): %s", Arrays.toString(params), result));
