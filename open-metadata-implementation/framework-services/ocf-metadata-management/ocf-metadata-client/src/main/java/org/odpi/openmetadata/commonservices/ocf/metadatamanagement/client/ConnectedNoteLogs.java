...
/**
 * ConnectedNoteLogs provides the open metadata concrete implementation of the
 * Open Connector Framework (OCF) NoteLogs abstract class.
...
public class ConnectedNoteLogs extends NoteLogs
{
    ...
    {
        final String   methodName = "NoteLogs.getCachedList";
        final String   urlTemplate = "/servers/{0}/open-metadata/framework-services/{1}/connected-asset/users/{2}/assets/{3}/note-logs?elementStart={4}&maxElements={5}";
            ...
            else
            {
                ...
                for (NoteLogResponse  noteLogResponse : noteLogResponses)
                {
                    if ((noteLogResponse != null) && (noteLogResponse.getNoteLog() != null))
                    {
                        ...
                        {
                            resultList.add(new NoteLog(noteLogResponse.getNoteLog(),
