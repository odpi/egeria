...
public class LicenseConverter<B> extends OCFConverter<B>
{
     ...
     */
    public LicenseConverter(OMRSRepositoryHelper repositoryHelper,
                                  String               serviceName,
                                  String               serverName)
    {
                        ...
                        String       methodName) throws PropertyServerException
    {
        try
        {
            ...
            if (returnBean instanceof License)
            {
                ...
                bean.setLicenseTypeName(entity.getType().getTypeDefName());
                ...
                if (relationship != null)
                {
                    ...
                    bean.setLicenseGUID(this.getCertificationGUID(instanceProperties));
                    bean.setLicenseConditions(getConditions(instanceProperties));
                    bean.setStartDate(getStart(instanceProperties));
                    bean.setEndDate(getEnd(instanceProperties));
                    bean.setCreatedBy(getLicensedBy(instanceProperties));
                    bean.setCustodian(getCustodian(instanceProperties));
                    bean.setLicensee(getLicensee(instanceProperties));
