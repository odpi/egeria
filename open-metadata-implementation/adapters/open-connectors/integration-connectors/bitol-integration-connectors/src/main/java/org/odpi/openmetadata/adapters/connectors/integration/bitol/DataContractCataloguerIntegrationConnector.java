...
/**
 * DataContractCataloguerIntegrationConnector registers a Bitol listener with the integration daemon and catalogues each
 * Open Data Contract Standard (ODCS) document it receives as an Agreement (classified as a DataSharingAgreement) in open
 * metadata, along with the contract's schema (as data structures and data fields), data quality checks (as data quality
 * rules), service level agreement (as a service level objective), servers (as endpoints), access roles, team, support
 * channels and authoritative definitions.  The mapping is performed by DataContractMapper from the Open Integration
 * Framework so that the same logic can be used by the Product Manager view service.
 */
public class DataContractCataloguerIntegrationConnector extends IntegrationConnectorBase implements BitolDocumentListener
{
    private DataContractMapper mapper = null;


    /**
     * Default constructor
     */
    public DataContractCataloguerIntegrationConnector()
    {
    }
    ...
    public void start() throws ConnectorCheckedException, UserNotAuthorizedException
    {
        ...
        {
            mapper = new DataContractMapper(integrationContext);
    ...
    public void processDataContract(DataContract dataContract,
                                    String       rawDocument)
    {
        final String methodName = "processDataContract";

        if ((dataContract == null) || (mapper == null))
        {
            /*
        ...
        try
        {
            ...
            BitolMappingResult result = mapper.catalogueDataContract(dataContract, connectorName);
