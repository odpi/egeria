 ...
 * that it works with.  DuckDB is embedded/in-process (a ".duckdb" file, or an in-memory ":memory:" session opened
 * directly through JDBC) so there is no server tier - only a single DATABASE target.
 */
public enum DuckDBTarget
{
    /**
     * DuckDB Database.
     */
    DATABASE("duckdbDatabase",
             DuckDBDeployedImplementationType.DUCKDB_DATABASE.getDescription(),
             DuckDBDeployedImplementationType.DUCKDB_DATABASE.getAssociatedTypeName(),
             DuckDBDeployedImplementationType.DUCKDB_DATABASE.getDeployedImplementationType(),
             null),
     ...
     */
    public static List<CatalogTargetType> getDuckDBDatabaseCatalogTargetTypes()
    {
        List<CatalogTargetType> catalogTargetTypes = new ArrayList<>();

        catalogTargetTypes.add(DATABASE.getCatalogTargetType());
    ...
    /**
     * Return the targets defined in this enum for a DuckDB database.
     *
     * @return list
     */
    public static List<ActionTargetType> getDuckDBDatabaseActionTargetTypes()
    {
