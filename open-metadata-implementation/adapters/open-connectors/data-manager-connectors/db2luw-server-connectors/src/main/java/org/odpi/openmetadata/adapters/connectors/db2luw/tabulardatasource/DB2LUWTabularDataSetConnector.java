...
import org.odpi.openmetadata.adapters.connectors.resource.jdbc.ddl.db2luw.DB2LUWSchemaDDL;
 ...
 */
public class DB2LUWTabularDataSetConnector extends ConnectorBase implements WritableTabularDataSource
{
    /*
     * Variables used for logging and debug.
     */
    private static final Logger log = LoggerFactory.getLogger(DB2LUWTabularDataSetConnector.class);
    ...
    public void start() throws UserNotAuthorizedException, ConnectorCheckedException
    {
        ...
        tableName = super.getStringConfigurationProperty(DB2LUWConfigurationProperty.TABLE_NAME.getName(),
                                                          connectionBean.getConfigurationProperties(),
                                                          "data");

        tableDescription = super.getStringConfigurationProperty(DB2LUWConfigurationProperty.TABLE_DESCRIPTION.getName(),
                                                                 connectionBean.getConfigurationProperties(),
                                                                 null);

        schemaName = super.getStringConfigurationProperty(DB2LUWConfigurationProperty.SCHEMA_NAME.getName(),
                                                           connectionBean.getConfigurationProperties(),
                                                           schemaName);

        schemaDescription = super.getStringConfigurationProperty(DB2LUWConfigurationProperty.SCHEMA_DESCRIPTION.getName(),
                                                                  connectionBean.getConfigurationProperties(),
                                                                  schemaDescription);
    ...
    private JDBCResourceConnector getDatabaseConnection() throws ConnectorCheckedException
    {
        ...
        throw new ConnectorCheckedException(DB2LUWErrorCode.NO_DATABASE_CONNECTION.getMessageDefinition(connectionBean.getDisplayName()),
                                            this.getClass().getName(),
                                            methodName);
    }
    ...
    /**
     * Return the table name qualified with its schema.  Db2 for Linux, UNIX and Windows has no equivalent of
     * PostgreSQL's "currentSchema=" JDBC URL parameter, so (unlike the PostgreSQL tabular data set connector)
     * table references in SQL statements issued by this connector are always schema-qualified explicitly,
     * rather than relying on the connection's default schema.
     *
    ...
    public void setColumnDescriptions(List<TabularColumnDescription> columnDescriptions) throws ConnectorCheckedException
    {
        ...
        try
        {
            ...
            {
                DB2LUWTable db2luwTable = new DB2LUWTabularTable(tableName,
                                                                  tableDescription,
                                                                  columnDescriptions);

                DB2LUWSchemaDDL db2luwSchemaDDL = new DB2LUWSchemaDDL(schemaName,
                                                                       schemaDescription,
    ...
    /**
     * Convert a list of values into an insert SQL statement.  As with Oracle, a trailing semicolon on a plain SQL
     * statement is not valid Db2 syntax outside of a compound (SQL PL) block, so none is added here.
     *
