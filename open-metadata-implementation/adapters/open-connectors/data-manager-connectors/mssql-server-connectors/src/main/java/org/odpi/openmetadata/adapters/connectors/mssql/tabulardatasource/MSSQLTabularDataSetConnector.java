...
import org.odpi.openmetadata.adapters.connectors.resource.jdbc.ddl.mssql.MSSQLSchemaDDL;
 ...
 */
public class MSSQLTabularDataSetConnector extends ConnectorBase implements WritableTabularDataSource
{
    /*
     * Variables used for logging and debug.
     */
    private static final Logger log = LoggerFactory.getLogger(MSSQLTabularDataSetConnector.class);
    ...
    public void start() throws UserNotAuthorizedException, ConnectorCheckedException
    {
        ...
        tableName = super.getStringConfigurationProperty(MSSQLConfigurationProperty.TABLE_NAME.getName(),
                                                         connectionBean.getConfigurationProperties(),
                                                         "data");

        tableDescription = super.getStringConfigurationProperty(MSSQLConfigurationProperty.TABLE_DESCRIPTION.getName(),
                                                                connectionBean.getConfigurationProperties(),
                                                                null);

        schemaName = super.getStringConfigurationProperty(MSSQLConfigurationProperty.SCHEMA_NAME.getName(),
                                                          connectionBean.getConfigurationProperties(),
                                                          schemaName);

        schemaDescription = super.getStringConfigurationProperty(MSSQLConfigurationProperty.SCHEMA_DESCRIPTION.getName(),
                                                                 connectionBean.getConfigurationProperties(),
                                                                 schemaDescription);
    ...
    private JDBCResourceConnector getDatabaseConnection() throws ConnectorCheckedException
    {
        ...
        throw new ConnectorCheckedException(MSSQLErrorCode.NO_DATABASE_CONNECTION.getMessageDefinition(connectionBean.getDisplayName()),
                                            this.getClass().getName(),
                                            methodName);
    }
    ...
    /**
     * Return the table name qualified with its schema.  Microsoft SQL Server has no equivalent of PostgreSQL's
    ...
    public void setColumnDescriptions(List<TabularColumnDescription> columnDescriptions) throws ConnectorCheckedException
    {
        ...
        try
        {
            ...
            {
                MSSQLTable mssqlTable = new MSSQLTabularTable(tableName,
                                                              tableDescription,
                                                              columnDescriptions);

                MSSQLSchemaDDL mssqlSchemaDDL = new MSSQLSchemaDDL(schemaName,
                                                                   schemaDescription,
